...
| Privileged Containers | `spec.containers[*].securityContext.privileged`,<br> init/ephemeral containers | `undefined/nil`, `false` | ✅ Enforced | Hardcoded: `privileged` must be absent or `false`. |
| Capabilities (`add`) | `spec.containers[*].securityContext.capabilities.add`,<br> init/ephemeral containers | `undefined/nil`, or one of 14 named capabilities | ✅ Stricter | Only `NET_BIND_SERVICE` may appear in `capabilities.add`. Baseline permits 13 additional capabilities (e.g. `CHOWN`, `KILL`, `SETUID`); the webhook rejects them all. `capabilities.drop` is not checked (required only by Restricted). |
...
| Host Ports | `spec.containers[*].ports[*].hostPort`,<br> init/ephemeral containers | `undefined/nil`, `0` | ✅ Enforced | Hardcoded: `hostPort` must be absent or `0` across all container types. |
| Host Probes / Lifecycle Hooks (v1.34+) | `httpGet.host` and `tcpSocket.host` on<br> liveness/readiness/startup probes and lifecycle hooks | `undefined/nil`, `""` | ❌ Not enforced | Probe and lifecycle hook `host` fields are not inspected. |
| `/proc` Mount Type | `spec.containers[*].securityContext.procMount`,<br> init/ephemeral containers | `undefined/nil`, `Default` | ✅ Enforced | Hardcoded: `procMount` must be absent, `""`, or `"Default"`. |
